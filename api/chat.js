...
// Busca estoque em tempo real via API Tecimob + recomendação via Anthropic
...
// Documentação: https://app.swaggerhub.com/apis-docs/tecimob/tecimob-api-publica/1.0.0
async function buscarEstoqueTecimob() {
  ...
  // Ajuste os nomes dos campos conforme o JSON real retornado pelo Tecimob.
  const imoveis = (data.data || data.properties || data || []).map((im) => ({
    ...
    // Usa lat/lon direto do Tecimob se disponível (mais rápido que geocodificar)
...
export default async function handler(req, res) {
  ...
  try {
    // 1. Busca estoque real do Tecimob
    ...
    if (originLat && originLon && raio) {
      ...
      const comCoord = await Promise.all(
        estoque.map(async (im) => {
          // Se o Tecimob já retornou lat/lon, usa diretamente
          if (im.lat && im.lon) {
            const dist = Math.round(haversineKm(originLat, originLon, im.lat, im.lon) * 10) / 10;
            return { ...im, dist };
          }
          // Caso contrário, geocodifica pelo CEP
          if (im.cep) {
      ...
      imovelFiltrados = comCoord
        ...
        .slice(0, 20); // limita para não estourar o contexto da IA
    }
...
Estoque real da Pena de Ouro (atualizado agora via Tecimob):
${estoqueStr}`;

    // 4. Chama a Anthropic com o estoque real injetado no contexto
    ...
    // 5. Retorna resposta da IA + lista de imóveis filtrados para o frontend
